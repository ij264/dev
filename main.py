...
for kernel_type in kernel_types:
    ...
    for l_max in l_maxes:
        for visc_src in tqdm(visc_srces):
            ...
            if not os.path.exists(f'{location}/S20RTS/l_max = {l_max}/{excised_top_layer_location}/{visc_src}/{kernel_type}/'):
                os.makedirs(f'{location}/S20RTS/l_max = {l_max}/{excised_top_layer_location}/{visc_src}/{kernel_type}/')
            ...
            DT_power = process.plot_power_spectrum(clm_DT, unit='km2', save=True,
                                                   fname=f'{location}/S20RTS/l_max = {l_max}/{excised_top_layer_location}/{visc_src}/{kernel_type}/power_spectrum')
            ...
            with fig.subplot(
                ...
                with fig.set_panel(panel=0):
                    ...
                    fig.grdcontour(xr_DT,
                                   ...
                                   projection='H12c',
                                   frame=True)
                    fig.coast(shorelines=True, resolution='c', area_thresh=1000, projection='H12c',
                              region=[0, 360, -90, 90])

                    fig.colorbar(frame=[f"a{annotation}f{interval}", "x+lElevation", "y+lm"])
                # elevation model
                with fig.set_panel(panel=1):
                    fig.plot(
                        ...
                        pen='1p',
                        region=[viscosity.min()/2, viscosity.max()*2, viscosity_radius.min(), viscosity_radius.max()]
                    )

            print(f'Saving {kernel_type}...')
            fig.savefig(f'{location}/S20RTS/l_max = {l_max}/{excised_top_layer_location}/{visc_src}/{kernel_type}/map.png', dpi=300)
            # print('Done. \n')
