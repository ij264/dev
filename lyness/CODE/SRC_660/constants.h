   ...
   real(dp),      parameter :: gearth=9.81
   real(qp),      parameter :: Emass=5.974e24
   real(qp),      parameter :: rhoearth=5.515e+03
   ...
   integer(i4b),  parameter:: kappaindex=0
   !> Starting Depth for calculating the gravity based on (-10)
   real(qp),      parameter:: top_depth =  000
   real(qp),      parameter:: bot_depth =  -010 
   !> boundary condition on top 
   integer(i4b),  parameter:: bdry=1
   !> boundary condition on bot 
   integer(i4b),  parameter:: bdry_bot=1
   !> background density
   real(qp),      parameter:: bgrho = 4.000e+03
   !> background Viscosity
   real(qp),      parameter:: visc0 = 10e21 
   !> background Advection factor
   real(qp),      parameter:: fac_ad = 3e10      
