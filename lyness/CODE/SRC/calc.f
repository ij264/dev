	  ...
	  real(dp) srfdt_lw_grd(grid , grid2)
	  real(dp) dynrf_lw_grd(grid , grid2)
      real(dp) cmbdt_lw_grd(grid , grid2)
      real(dp) GeoCorr, GravCorr, DyntCorr, flatness, CMBCorr
      real(dp) start, finish, obs_surmax, obs_geomax, CMB_ell
      real(dp) laysize
     ...
     &                           *leg(l,m,i)*sin(m*phi(j))
			   ...
			   if (l==2) then
 				  srfdt_lw_grd(i,j)=srfdt_lw_grd(i,j)+SurTopCO(l,m)
     &                *sphharm
	              dynrf_lw_grd(i,j)=dynrf_lw_grd(i,j)+dynrfCO(l,m)
     &                *sphharm
			      if (m==0) then
                     cmbdt_lw_grd(i,j)=cmbdt_lw_grd(i,j)+CMBTopCO(l,m)
     &                *sphharm
				  end if
			   end if
      ...
      call mpi_synch_fld(srfdt_lw_grd)
      call mpi_synch_fld(cmbdt_lw_grd)
     ...
     &    (sqrt(sum(cmbrfCO**2)*sum(CMBTopCO**2)))
         ...
         obs_geomax = maxval(abs(georf_grd))
   	     CMB_ell=-CMBTopCO(2,0)*sqr(2*2+1)*0.5*sqpi*
     &    (maxval(leg(2,0,:))-minval(leg(2,0,:)))
         !flatness  =
!     &   ! sqrt(sum(SurTopCO(2:3,:)**2)/sum(SurTopCO(8: 9,:)**2))
C 		 print *,maxval(cmbdt_lw_grd),minval(cmbdt_lw_grd),
C      &    maxval(srfdt_lw_grd),minval(srfdt_lw_grd)
C      &    maxval(geo_grd),minval(srfdt_grd),
C      &    maxval(srfdt_grd),minval(grv_grd)
C      &    ,maxval(grv_grd)
C 		stop
      endif
      ...
      if (inv_flg==0) then      
         ...
         call fldout(srfdt_lw_grd, 'GRD_surftopo_l2'   )
         call fldout(cmbdt_lw_grd, 'GRD_cmbtopo_l2_m0' )
         ...
         call fldout(dynrf_lw_grd,'GRD_REF_surftopo_l2')    
